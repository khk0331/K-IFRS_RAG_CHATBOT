...
회계·감사 업무에서 중요한 것은 답변 자체보다 그 답변이 어떤 기준서 문단에 근거하는지 확인하는 것입니다. 그러나 기준서 PDF를 직접 검색하면 질문 표현과 기준서 용어가 다를 때 관련 문단을 놓치기 쉽고, 생성형 AI를 그대로 사용하면 근거가 불명확한 답변이 섞일 수 있습니다.

K-IFRS RAG는 이 문제를 해결하기 위해 K-IFRS 문단을 구조화하고, 검색된 근거 문단 안에서만 답변하도록 제한한 회계기준서 검색·답변 챗봇입니다.

## Executive Summary

- **Problem**: 감사인은 기준서 근거를 빠르게 확인해야 하지만, PDF 검색만으로는 질문과 직접 연결되는 문단을 안정적으로 찾기 어렵습니다.
- **Solution**: K-IFRS 문단을 기준서 번호, 문단 ID, 페이지 정보와 함께 구조화하고, Dense 검색과 BM25 검색을 결합한 RAG 파이프라인을 구현했습니다.
- **Output**: 질문에 대한 답변, 기준서 번호, 문단 ID, 원문 근거, 답변 보류 상태를 함께 제공합니다.
- **Scope**: 회계 판단을 대신하는 챗봇이 아니라, 기준서 근거 확인과 조서 작성 전 검토를 보조하는 도구입니다.

## Retrieval Failures Addressed

이 프로젝트는 단순히 "검색 후 답변"을 붙인 챗봇이 아니라, 회계기준서 검색에서 자주 생기는 실패경로를 줄이는 데 초점을 두었습니다.

| Failure path | Control |
| --- | --- |
| 질문 표현과 기준서 용어가 달라 관련 문단을 놓침 | Dense embedding과 BM25를 결합해 의미 유사도와 키워드 일치를 함께 사용 |
| 기준서 번호나 문단 번호가 답변 근거에서 분리됨 | PDF를 문단 단위로 파싱하고 chunk metadata에 기준서·문단·페이지 정보 보존 |
| 키워드 라우팅이 너무 강해 정답 후보를 잘라냄 | 기준서 힌트를 hard filter가 아니라 reranking bonus로 사용 |
| 생성 모델이 검색되지 않은 근거를 인용함 | 서버가 evidence ID를 검증하고, 후보에 없는 인용은 `validation_failed`로 차단 |
| 관련 근거가 부족한 질문에 억지 답변을 생성함 | 최소 점수 임계값 미달 시 `insufficient_evidence`로 답변 보류 |

## Project Context

이 저장소는 기준서 근거를 정확히 확인하기 위한 프로젝트입니다. 함께 개발한 Audit Risk Radar가 "감사인이 특정 회사를 어떤 관점에서 먼저 이해할 것인가"에 초점을 둔다면, K-IFRS RAG는 "감사인이 판단의 근거가 되는 기준서 문단을 어떻게 확인할 것인가"에 초점을 둡니다.

두 프로젝트는 각각 회사 리스크 분석과 기준서 근거 확인이라는 감사 업무의 서로 다른 문제를 다룹니다.
...
실제 데이터 기준 51개 기준서, 14,548개 검색 청크와 384차원 다국어 임베딩을 사용했습니다. GPT 하네스는 질문을 검색 질의로 분해하고, 모호한 질문에는 추가 정보를 요청하며, 검색 후보 중 선택한 문단만 답변 근거로 반환합니다.
...
구현은 [PDF 파서](scripts/ingest_pdfs.py), [인덱스 생성기](scripts/build_index.py), [하이브리드 검색·재순위화](src/kifrs_rag/hybrid_retrieval.py), [GPT 하네스](src/kifrs_rag/openai_harness.py), [API와 인용 검증](src/kifrs_rag/api.py)에서 확인할 수 있습니다. 공개 저장소에서는 저작권과 보안을 위해 K-IFRS 원문·파생 인덱스·API 키를 제외했으며, 실제 모드 실행 코드는 포함합니다.

## Evaluation Snapshot

공개 데모 데이터는 합성 문단 8개만 포함하므로 실제 K-IFRS 검색 성능을 대표하지 않습니다. 대신 파이프라인이 근거 검색, 답변 가능성 판단, 인용 검증을 수행하는지 확인하는 smoke test 역할을 합니다.

| Evaluation set | Data mode | Purpose | Current result |
| --- | --- | --- | --- |
| `evals/baseline.jsonl` | public synthetic demo | 기본 검색·답변 보류 회귀 테스트 | Recall@3 `1.00`, answerability accuracy `1.00` |
| `evals/hybrid_regression.jsonl` | private K-IFRS index required | 실제 기준서 문단 검색 회귀 테스트 | 로컬 실제 인덱스 보유 시 실행 |
| `evals/hybrid_confusion_pairs.jsonl` | private K-IFRS index required | 리스/손상, 재고/손상처럼 혼동되는 기준서 쌍 점검 | 로컬 실제 인덱스 보유 시 실행 |

위 수치는 공개 데모 기준의 제한된 회귀 결과입니다. 합성 문단 수가 작기 때문에 성능 수치 자체보다 실패 시 답변을 보류하고, 인용을 서버에서 검증하는 통제 흐름을 확인하는 목적이 큽니다.
...
실제 모드는 임베딩 검색 순위와 BM25 검색 순위를 Reciprocal Rank Fusion 방식으로 결합합니다. 이후 질문에서 식별한 회계 주제, 최초·후속 측정 의도, 기준서 범위와 특수 산업 용어를 반영해 후보 점수를 다시 계산합니다. 예를 들어 `사용권자산의 후속 측정` 질문에는 리스 기준서의 감가상각·손상·재측정 문단을 우대하고, 질문에 없는 농림어업이나 보험 문단은 감점합니다.

초기 버전은 질문에 `사용권자산`, `현금흐름창출단위` 같은 강한 힌트가 있으면 해당 기준서 후보만 남기는 hard filter를 사용했습니다. 현재 버전은 이를 reranking bonus로 낮췄습니다. 이 변경은 명확한 질문에서는 관련 기준서를 우대하되, 여러 기준서가 동시에 관련되는 질문에서 정답 후보가 초기에 제거되는 위험을 줄이기 위한 것입니다.

공개 합성 데모 기준 `evals/baseline.jsonl`의 수정 후 결과는 Recall@3 `1.00`, MRR `1.00`, answerability accuracy `1.00`입니다. 이 수치는 작은 데모셋에 대한 회귀 확인이며, 실제 K-IFRS 인덱스의 일반화 성능을 의미하지 않습니다.
...
플래너와 답변 모델은 환경변수 또는 코드 설정으로 지정합니다. 누적 예상비용이 설정된 한도에 도달하면 추가 호출을 차단합니다. 사용량은 Git에서 제외된 `data/private/openai_usage.json`에 기록하며 API 키는 저장하지 않습니다.
...
- 폐쇄망 또는 온프레미스 환경에서는 PDF, 인덱스, API 서버를 내부망에 두고 외부 생성 모델 호출을 비활성화한 extractive 모드로 운영할 수 있습니다.
